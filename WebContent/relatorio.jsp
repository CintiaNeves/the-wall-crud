...
<head>
	...
	<script type="text/javascript">
		...
		function drawCharts() {
			graficoColunas("Produtos vendidos no per�odo", "bar_chart_div", { operacao: "CONSULTAR", relatorio: "ITENS_VENDIDOS", retornoJson: true });
		}
		...
		function graficoColunas(title, chartElement, data) {
			...
			$.ajax({
				...
				data,
				success: relatorio => {
					// objeto de dados do gr�fico do google
			...
			document.getElementById("consultar").addEventListener("click", event => {
				...
				if (inicio.value && fim.value) {
					if (new Date(inicio.value).getTime() > new Date(fim.value).getTime()) {
						erro = true;
						mensagem.innerText = "Data fim �o poder ser maior que a data início!";
				...
				if (!erro) { // realiza consulta
					...
					dataFim = fim.value;
					graficoColunas("Produtos vendidos no per�odo", "bar_chart_div",
...
<body id="body">
	...
	<div hidden style="margin: 0% 2%;" class="row">
		<div class="form-group col-xs-4 col-md-4">
			<label for="inicio" class="control-label">Per�do In�cial</label>
			<input type="date" class="form-control" id="inicio" name="inicio">
		</div>
		<div class="form-group col-xs-4 col-md-4">
			<label for="fim" class="control-label">Per�odo Final</label>
