					...
					<form class="row contact_form" action="ocorrencia" method="POST">
						...
						<div style="${param['ativo'] ? 'display:none' : ''}"
							...
							<label class="col-md-12" for="categoria-ativacao">Categoria
								...
								<option value="1"
									${ocorrencia.tipo.id == 1 ? 'selected="selected"' : ''}>Entrada
									em estoque</option>
								<option value="2"
									${ocorrencia.tipo.id == 2 ? 'selected="selected"' : ''}>Demanda
									do mercado</option>
							</select>
						</div>
						<div></div>
						<div style="${!param['ativo'] ? 'display:none' : ''}"
							...
							<label class="col-md-12" for="categoria-inativacao">Categoria
								...
								<option value="">Selecione...</option>
								<option value="3"
									${ocorrencia.tipo.id == 3 ? 'selected="selected"' : ''}>Sem
									estoque</option>
								<option value="4"
									${ocorrencia.tipo.id == 4 ? 'selected="selected"' : ''}>Fora
									de linha</option>
								<option value="5"
									${ocorrencia.tipo.id == 5 ? 'selected="selected"' : ''}>Sem
									contrato com fornecedor</option>
							</select>
						</div>
						<div class="col-md-12 form-group p_star">
							<label for="justificativa">Justificativa</label>
							<textarea id="justificativa" name="justificativa" class=form-control>${ocorrencia.justificativa}</textarea>
						</div>
...
<script>
$("#categoria-ativacao").change(function() {
	$("#mensagem").html("");
...
$("#categoria-inativacao").change(function() {
	$("#mensagem").html("");
	$("#mensagem").removeClass("alert");
	$("#mensagem").removeClass("alert-danger");
	$("#backend_menssagem").html("");
